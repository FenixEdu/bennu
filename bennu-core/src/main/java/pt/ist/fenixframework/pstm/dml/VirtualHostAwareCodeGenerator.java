...
public class VirtualHostAwareCodeGenerator extends FenixCodeGeneratorOneBoxPerObject {
    ...
    @Override
    protected void generateRoleSlotMethodsMultStarCount(final Role role, final PrintWriter out,
	    final String methodModifiers, final String capitalizedSlotName, final String slotAccessExpression) {
	...
	final String slotExpression = wrapSlotExpression(role, slotAccessExpression);
	super.generateRoleSlotMethodsMultStarHasChild(role, out, methodModifiers, capitalizedSlotName, slotExpression,
		typeName, slotName, isIndexed, indexGetterCall);
    }
	...
	super.generateRoleSlotMethodsMultStarSet(role, out, methodModifiers, capitalizedSlotName, slotExpression, slotName, typeName);
    }
	...
	if (type instanceof DomainClass) {
		...
		if (!isVirtualHostSpecific(thisDomainClass)) {
		    final String wrapperType = makeGenericType("pt.ist.bennu.core.domain.VirtualHostFilterCollectionWrapper", typeFullName);
	...
	return superclass != null
		&& superclass instanceof DomainClass
		&& implementsVirtualHost((DomainClass) superclass);
    }
