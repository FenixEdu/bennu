...
public class FunctionalityFilter implements Filter {
	...
	try {
	    ...
	    for (FenixFrameworkArtifact artifact : FenixFrameworkArtifact.fromName(PropertiesManager.getProperty("app.name"))
		    .getArtifacts()) {
		try (InputStream stream = loader.getResourceAsStream(artifact.getName() + File.separatorChar
			...
			for (String funcMapping : mappings) {
			    ...
			    final FunctionalityInfo functionality = new FunctionalityInfo(mappingComponents[0],
				    mappingComponents[1], mappingComponents[2].equals("-") ? null : mappingComponents[2]);
