...
public class GroupResource extends BennuRestResource {
    ...
    @Path("/members")
    @Produces(MediaType.APPLICATION_JSON)
    public Response members(@QueryParam("groupExpression") String groupExpression) {
        accessControl(Group.managers());
        return Response.ok(view(Group.parse(groupExpression), GroupMemberJsonAdapter.class)).build();
    }
    ...
    public static class GroupMemberJsonAdapter implements JsonViewer<Group> {
        ...
        @Override
        public JsonElement view(final Group obj, final JsonBuilder ctx) {
            final JsonObject json = new JsonObject();
            json.addProperty("name", obj.getPresentationName());
            json.addProperty("expression", obj.getExpression());
            final JsonArray members = obj.getMembers()
                    .map(user -> new JsonPrimitive(user.getUsername()))
                    .collect(StreamUtils.toJsonArray());
