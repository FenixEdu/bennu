...
app.config(['$stateProvider', '$urlRouterProvider',
    function ($stateProvider, $urlRouterProvider) {
        ...
        // Domain Browser
        $stateProvider.state('domain-browser', {
            url: '/domain-browser',
        ...
        // Portal
        $stateProvider.state('portal-configuration', {
...
app.filter('uptime', function () {
    ...
    return function (value) {
        value = value || 0;
        var days = Math.floor(value / (24 * 60 * 60)), hours = Math.floor(value / (60 * 60)) % 24,
...
function fallbackCopy(text, $event, ftn) {
...
function showTooltip($event) {
    ...
    setTimeout(function() {
        if (tooltip && tooltip.parentNode === elem) {
...
app.controller('IOController', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
        ...
        $http.post(contextPath + '/api/bennu-io/storage/lfs',
            {
                'name': $scope.fsName,
                'path': $scope.path,
                'treeDirectoriesNameLength': $scope.treeLen
            }).success(function (data) {
            $scope.fsName = $scope.path = $scope.treeLen = null;
            $scope.storages.push(data);
            $scope.counts[data.id] = 0;
        }).error(function (data) {
            $scope.error = data;
    ...
    $scope.newDriveStorage = function () {
        $http.post(contextPath + '/api/bennu-io/storage/drive',
            {
                'name': $scope.driveName,
                'driveUrl': $scope.driveUrl,
                'remoteUsername': $scope.remoteUsername,
                'remoteDirectoryId': $scope.remoteDirectoryId
            }).success(function (data) {
            $scope.driveName = $scope.driveUrl = $scope.remoteUsername = $scope.remoteDirectoryId = null;
            $scope.storages.push(data);
            $scope.counts[data.id] = 0;
        }).error(function (data) {
            $scope.error = data;
...
app.controller('JMXController', ['$scope', '$http', function ($scope, $http) {
    $http.get(contextPath + '/api/bennu-core/system/jmx').success(function (data) {
        $scope.jmxData = data;
    });
}]);
...
app.controller('MenuController', ['$scope', '$state', '$http', function ($scope, $state, $http) {
    ...
    $scope.saveSelected = function () {
        var data = {
            ...
            isEntryPoint: $scope.selected.isEntryPoint,
            layout: $scope.selected.layout,
            accessExpression: $scope.selected.accessExpression,
            icon: $scope.selected.icon,
            documentationUrl: $scope.selected.documentationUrl,
            supportConfig: $scope.selected.supportConfig.id,
            faqUrl: $scope.selected.faqUrl
        };
        var promise;
        if ($scope.selected.id) {
            ...
            data.path = $scope.selected.path;
            data.parent = $scope.selected.parent;
    ...
    $scope.installApplication = function () {
        $http.post(contextPath + "/api/bennu-portal/menu/applications/", {
            root: $scope.selected.id,
            key: $scope.newApp.key
    ...
    $scope.installFunctionality = function () {
        $http.post(contextPath + '/api/bennu-portal/menu/functionalities/',
            {
                root: $scope.selected.id,
                appKey: $scope.newApp.key,
                provider: $scope.newFunctionality.provider,
                key: $scope.newFunctionality.key
            }).success(function (item) {
            add(item, $scope.selected.node);
        }).error($scope.handleError);
    }
    $scope.createChild = function (root) {
        ...
        var newChild = {
            ...
            subRoot: root,
            supportConfig: $("#tree").fancytree("getTree").getFirstChild().data.item.supportConfig,
            supportConfigs: $("#tree").fancytree("getTree").getFirstChild().data.item.supportConfigs
        };
    ...
    $scope.addSupport = function () {
        $http.post(contextPath + '/api/bennu-portal/menu/' + $scope.selected.id + '/support/',
            {title: $scope.newSupportTitle, email: $scope.newSupportEmail}).success(function (data) {
            var node = $scope.selected.node;
            $scope.selected = data;
            data.node = node;
            node.setTitle(i18n(data.title));
    ...
    var add = function (item, parent) {
        var node = parent.addChildren({
            title: (item.subRoot ? '[MENU] ' : '') + i18n(item.title),
            key: item.id,
            folder: !item.functionality,
            item: item
        });
        item.node = node;
        if (item.menu) {
...
app.controller('GroupsController', ['$scope', '$http', function ($scope, $http) {
    ...
    function doIt(op, user) {
        return $http.post(contextPath + '/api/bennu-core/groups/dynamic/' + op, {
            'group': $scope.selectedGroup.name,
            'user': user.username
        }).success(function (data) {
            $scope.groups[$scope.groups.indexOf($scope.selectedGroup)] = data;
    ...
    $scope.grant = function (user) {
        doIt('grant', user).success(function () {
            $scope.error = false;
        }).error(function () {
            $scope.error = true;
        });
    ...
    $scope.saveName = function () {
        $http.post(contextPath + '/api/bennu-core/groups/dynamic',
            {
                'group': $scope.selectedGroup.name,
                'name': $scope.selectedGroup.customPresentationName
            }).success(function (data) {
            $scope.groups[$scope.groups.indexOf($scope.selectedGroup)] = data;
            $scope.selectedGroup = data;
        });
...
app.controller('TasksController', ['$scope', '$http', '$state', '$timeout', function ($scope, $http, $state, $timeout) {
    ...
    $scope.addSchedule = function () {
        ...
        $http.post(contextPath + '/api/bennu-scheduler/schedule', {
            'type': $scope.type,
...
app.controller('LogDetailsController', ['$scope', '$http', '$state', '$interval', function ($scope, $http, $state, $interval) {
    ...
    $scope.load = function () {
        window.code = $scope.log.code;
        $state.go('scheduler.custom-new');
    };
    $scope.copyCode = function($event) {
        var codeText = $scope.log.code || '';
        return copy(codeText, $event);
    };
    $scope.copyLogs = function($event) {
        var logsText = $scope.logs || '';
        return copy(logsText, $event)
    };
...
app.controller('CustomTaskController', ['$scope', '$http', '$state', function ($scope, $http, $state) {
    $scope.javaCode = window.code || "";
    window.code = null;
    $scope.compiled = false;
    $scope.editorOptions = {lineNumbers: true, mode: 'text/x-java', theme: 'eclipse'};
    $scope.compile = function () {
        ...
        $http.put(contextPath + '/api/bennu-scheduler/custom/compile', {
            'name': fqn,
            'code': $scope.javaCode
        }).success(function (result) {
            if (result.compileOK) {
                $scope.result = 'The compilation was successful!';
                $scope.compiled = true;
            } else {
    ...
    $scope.run = function () {
        ...
        $http.put(contextPath + '/api/bennu-scheduler/custom', {
            'name': fqn,
            'code': $scope.javaCode
        }).success(function (result) {
            $state.go('scheduler.logs')
...
app.controller('CustomTaskLogsController', ['$scope', '$http', '$state', function ($scope, $http, $state) {
    ...
    $scope.reload = function () {
        $http.get(url).success(function (data) {
            ...
            angular.forEach($scope.logs, function (value) {
                value.date = moment(value.begin).format("DD MMMM YYYY HH:mm:ss");
            });
        });
    }
    $scope.reload();
