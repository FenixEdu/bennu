    ...
    Bennu.htmlEditor.components.attachModal = function () {
        if (!$("#bennu-html-editor-component-modal").length) {
            var template = '<div class="modal fade" id="bennu-html-editor-component-modal">' +
                ...
                '<button type="button" class="close" data-dismiss="modal"><span class="sr-only">Cancel</span></button>' +
            ...
            if (onlyMedia) {
                if (c.category === "media") {
                    ...
                    if (c.resources.length > 0) {
                        logo = Bennu.contextPath + c.resources[0];

                        output += '<li class="list-group-item">' +
                            '<h4>' +
                            '<a href="#"><img data-type="' + c.key + '" alt="" src="' + logo + '" height="25px"></a>' +
                            '</h4>' +
                            '<p></p>' +
                            '<p>' + c.description + '</p>' +
                            '</li>';
        ...
        Bennu.ensure([
            ...
            for (var i = 0; i < toolbarReqs.length; i++) {
                ...
                if (c === "size") {
                    ...
                    $(".btn-toolbar", dom).append('<div class="btn-group">' +
                        '<a class="btn btn-sm btn-default link-to-add-btn dropdown-toggle" data-toggle="dropdown" title="Insert Hyperlink" data-original-title="Hyperlink"><span class="glyphicon glyphicon-link"></span></a>' +
                    ...
                    $(".btn-toolbar", dom).append('<div class="btn-group">' +
                        '<button type="button" class="btn table-btn btn-default btn-sm btn-small dropdown-toggle" title="Insert Table" data-original-title="Table"><i class="fa fa-table icon-table"></i></button>' +
                        '</div>');
                } else if (c === "media") {
                    $(".btn-toolbar", dom).append('<div class="btn-group">' +
                    ...
                    $(".btn-toolbar", dom).append('<div class="btn-group">' +
                        '<a class="pictureBtn btn btn-sm btn-default" title="" data-original-title="Insert Image"><span class="glyphicon glyphicon-picture"></span></a>' +
                        '</div>');
                } else if (c === "undo") {
                    $(".btn-toolbar", dom).append('<div class="btn-group">' +
            ...
            if ($(".bennu-html-code-editor", dom).length != 0) {
                ...
                $(".switch-to-code", dom).on("click", function () {
                    setupEditor(dom, function (editor) {
                        if (!dom.data("showSrc")) {
                            $(".bennu-html-editor-toolbar .btn", dom).map(function () {
                                (!$(this).hasClass("switch-to-code")) && $(this).attr("disabled", "");
                            });
